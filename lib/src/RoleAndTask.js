...
function () {
  ...
  function RoleAndTask() {
    (0, _classCallCheck2["default"])(this, RoleAndTask);
    if (instance) return instance; //
    // Mandatory to fill
    //
    // Set the Master Slave Configuration File to load

    this.launchMasterSlaveConfigurationFile = false; // Path to the entry point of your program, we use to pop a new slave

    this.pathToEntryFile = false; // The task we use to perform the displays : The task must be in master! If no task is provided here, the display is made to stdout
    ...
    this.displayLog = true; // Do we makes the error to be fatal ? One error -> Exit

    this.makesErrorFatal = _CONSTANT["default"].MAKES_ERROR_FATAL; // Do we consider warning as errors ?

    this.considerWarningAsErrors = _CONSTANT["default"].CONSIDER_WARNING_AS_ERRORS; // The amount of time a master wait for a slave message before to timeout

    this.masterMessageWaitingTimeout = _CONSTANT["default"].MASTER_MESSAGE_WAITING_TIMEOUT; // The amount of time a master wait for a slave message to acknowledge the state change before to timeout

    this.masterMessageWaitingTimeoutStateChange = _CONSTANT["default"].MASTER_MESSAGE_WAITING_TIMEOUT_STATE_CHANGE; // The amount of time a master wait for a slave message before to timeout

    this.masterMessageWaitingTimeoutStopTask = _CONSTANT["default"].MASTER_MESSAGE_WAITING_TIMEOUT_STOP_TASK; //
    //
    // Contains all the tasks referenced

    this.tasks = (0, _toConsumableArray2["default"])(Object.keys(_CONSTANT["default"].DEFAULT_TASK).map(function (x) {
      return _CONSTANT["default"].DEFAULT_TASK[x];
    })).filter(function (x) {
      return x.id !== -1;
    }); // Contains all the roles referenced

    this.roles = (0, _toConsumableArray2["default"])(Object.keys(_CONSTANT["default"].DEFAULT_ROLES).map(function (x) {
      return _CONSTANT["default"].DEFAULT_ROLES[x];
    })).filter(function (x) {
      return x.id !== -1;
    }); // Contains all the states the system can have

    this.states = (0, _toConsumableArray2["default"])(Object.keys(_CONSTANT["default"].DEFAULT_STATES).map(function (x) {
      return _CONSTANT["default"].DEFAULT_STATES[x];
    })); // Array where we store the functions to call when the state change

    this.stateChangeCallbacks = []; // The state of program patform

    this.programState = _CONSTANT["default"].DEFAULT_STATES.LAUNCHING; // All the orders in a row to change the program state

    this.programStateChangeWaitingList = []; // When poping a new process, we start it using a "launching mode", there are two basic launching mode for "slave" and "master"
    // You can set up a custom launching mode

    this.customLaunchingMode = []; // Are we quitting?

    this.quitOrder = false; // Contains the functions to call to validate mutex take and release in master/slave protocol

    this.masterMutexValidationFunctions = []; // Handle the signals such as SIGINT, SIGTERM...
  ...
  (0, _createClass2["default"])(RoleAndTask, [{
    ...
    value: function getProgramStateChangeToTreat() {
      ...
      this.programStateChangeWaitingList.some(function (x) {
        ...
        if (x.programState.id === _CONSTANT["default"].DEFAULT_STATES.ERROR.id) {
          errorElement = x;
          return true;
        }
      ...
      if (errorElement) return errorElement; // Then in progress

      if (inProgress) return false; // Then regular
            ...
            return _regenerator["default"].wrap(function _callee$(_context) {
              while (1) {
                switch (_context.prev = _context.next) {
                  case 0:
                    elementToTreat = _this2.getProgramStateChangeToTreat(); // Nothing to do
                  ...
                  case 3:
                    elementToTreat.inProgress = true; // If the state is already the good one
                  ...
                  case 12:
                    ...
                    if (!(role.id === _CONSTANT["default"].DEFAULT_ROLES.MASTER_ROLE.id)) {
                      _context.next = 20;
                      break;
                    }
                  ...
                  case 20:
                    // Say to everyone which is listening that the state changed
                    _this2.spreadStateToListener(); // If we are the slave - Do nothing else here (we just set the this.programState)
    ...
    value: function boot() {
      ...
      }).initialization(); // Get the instances of the roles class before to push it into the roleHandler
      ...
      this.roleHandler = new _RoleHandler["default"](this.roles);
    ...
    value: function subscribeToStateChange(callback) {
      var descriptor = _Utils["default"].generateLittleID();
              ...
              while (1) {
                switch (_context2.prev = _context2.next) {
                  case 0:
                    ...
                    throw new _Errors["default"]('EXXXX', 'No role available');
    ...
    value: function getSlaveNorMaster() {
      ...
      return new _PromiseCommandPattern["default"]({
        func: function func() {
          return _Utils["default"].promiseCallUntilTrue({
              ...
              while (1) {
                switch (_context3.prev = _context3.next) {
                  ...
                  case 3:
                    ...
                    isString = _Utils["default"].isAString(param.str);
                    ...
                    return _context3.abrupt("return", role.displayMessage(_objectSpread({}, param, {
                  ...
                  case 7:
                    newParam = _objectSpread({}, param, {
                      // Add the task who perform the display
                      from: RoleAndTask.getTheTaskWhoPerformTheDisplay(role),
                      time: Date.now()
                    });
                    newParam.str = JSON.stringify(newParam.str, null, 2); // Add here the task who performed the display and the time of it
              ...
              while (1) {
                switch (_context4.prev = _context4.next) {
                  ...
                  case 7:
                    _context4.prev = 7;
                    _context4.next = 10;
                    return _this7.changeProgramState(_CONSTANT["default"].DEFAULT_STATES.ERROR.id);
                  ...
                  case 14:
                    ...
                    RoleAndTask.exitProgramMsg('Exit program unproper ERROR HAPPENED', err, _context4.t0); // We use setTimeout tho if there is some others things to do before the quit it will
                  ...
                  case 20:
                    ...
                    RoleAndTask.exitProgramMsg('Exit program unproper ERROR HAPPENED CATCH', err, _context4.t1); // We use setTimeout tho if there is some others things to do before the quit it will
              ...
              while (1) {
                switch (_context5.prev = _context5.next) {
                  ...
                  case 5:
                    ...
                    throw new _Errors["default"]('EXXXX', 'Closure not possible in a slave');

                  case 8:
                    _context5.next = 10;
                    return _this8.changeProgramState(_CONSTANT["default"].DEFAULT_STATES.CLOSE.id);
            ...
            return _regenerator["default"].wrap(function _callee6$(_context6) {
              while (1) {
                switch (_context6.prev = _context6.next) {
                  case 0:
                    if (!(_this9.programState.id === _CONSTANT["default"].DEFAULT_STATES.LAUNCHING.id)) {
                      _context6.next = 3;
                      break;
                    }
                  ...
                  case 6:
                    quit = _context6.sent;
                    if (quit) RoleAndTask.exitProgramGood(); // Do nothing if quit equal to false
                    // ...
    ...
    value: function handleSignals() {
      ...
      Object.keys(_CONSTANT["default"].SIGNAL).forEach(function (x) {
        process.on(_CONSTANT["default"].SIGNAL[x], function () {
          return signalActionProper();
        });
      });
      Object.keys(_CONSTANT["default"].SIGNAL_UNPROPER).forEach(function (x) {
        process.on(_CONSTANT["default"].SIGNAL_UNPROPER[x], function () {
    ...
    value: function spreadDataToEveryLocalTask(_ref3) {
      var _this11 = this;

      var dataName = _ref3.dataName,
          data = _ref3.data,
          timestamp = _ref3.timestamp,
          limitToTaskList = _ref3.limitToTaskList;
      return new _PromiseCommandPattern["default"]({
        func: function () {
            ...
            return _regenerator["default"].wrap(function _callee10$(_context10) {
              while (1) {
                switch (_context10.prev = _context10.next) {
                  case 0:
                    _context10.next = 2;
                    return _this13.getRoleHandler().getRole(_CONSTANT["default"].DEFAULT_ROLES.MASTER_ROLE.id);
                  ...
                  case 2:
                    ...
                    throw new _Errors["default"]('EXXXX', 'Master is not active in getMasterRole');
    ...
    value: function setConfiguration(opts) {
      ...
      var availableOpts = ['mode', 'modeoptions', 'displayTask', 'launchMasterSlaveConfigurationFile', 'pathToEntryFile', 'displayLog', 'makesErrorFatal', 'considerWarningAsErrors', 'masterMessageWaitingTimeout', 'masterMessageWaitingTimeoutStateChange', 'masterMessageWaitingTimeoutStopTask'];
      var mandatoryOpts = ['mode', 'modeoptions', 'launchMasterSlaveConfigurationFile', 'pathToEntryFile'].reduce(function (tmp, x) {
        tmp[x] = null;
        return tmp;
      }, {});
      var setted = Object.keys(opts).reduce(function (tmp, x) {
        // Unknown key
        if (!availableOpts.includes(x)) return tmp; // Set the option value
      ...
      if (Object.values(mandatoryOpts).includes(null)) {
        throw new Error("Mandatory option ".concat(Object.keys(mandatoryOpts).find(function (x) {
          return mandatoryOpts[x] === null;
        }), " is missing"));
    ...
    key: "releaseMutex",
    value: function () {
      var _releaseMutex = (0, _asyncToGenerator2["default"])(
      /*#__PURE__*/
        ...
        return _regenerator["default"].wrap(function _callee14$(_context14) {
          while (1) {
            switch (_context14.prev = _context14.next) {
              case 0:
                return _context14.abrupt("return", new _PromiseCommandPattern["default"]({
                  func: function () {
      ...
      function releaseMutex(_x) {
        return _releaseMutex.apply(this, arguments);
      }
    ...
    key: "watchMemoryUsage",
    value: function watchMemoryUsage() {
      var lastWarning = Date.now();
      var lastReport = Date.now();
      setInterval(function () {
        var _v8$getHeapStatistics = _v["default"].getHeapStatistics(),
            total_heap_size = _v8$getHeapStatistics.total_heap_size,
            heap_size_limit = _v8$getHeapStatistics.heap_size_limit;

        var percentageUsed = total_heap_size * 100 / heap_size_limit; // Show a warning every 30 sec if a process is too high in memory usage

        if (percentageUsed > 60 && Date.now() - lastWarning > 30000) {
          lastWarning = Date.now();
          console.error("Warning: The memory consumption is reaching ".concat(percentageUsed, "% - ELIOT will shut down at soon at it reaches 90+% to prevent memory allocation failure"));
          console.error("Memory used :: Using ".concat(total_heap_size, " / ").concat(heap_size_limit));
        }

        if (percentageUsed > 90) {
          console.error("Error: The memory consumption is reaching ".concat(percentageUsed, "% - ELIOT shut down to prevent memory allocation failure"));
          console.error("Memory used :: Using ".concat(total_heap_size, " / ").concat(heap_size_limit));
    ...
    value: function getTheTaskWhoPerformTheDisplay(role) {
      ...
      if (!activeTasks.length) return "".concat(process.pid);
    ...
    value: function exitProgramUnproperDueToError() {
      ...
      }, _CONSTANT["default"].TIMEOUT_LEAVE_PROGRAM_UNPROPER);
    }
    /**
